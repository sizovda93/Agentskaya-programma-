...
const STORAGE_KEY = "learning_read_agent";
...
export default function AgentLessonPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const result = getLesson("agent", slug);
  const allLessons = getAllLessons("agent");
  ...
  if (!result) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        ...
        <Button variant="outline" onClick={() => router.push("/agent/learning")}>
  ...
  const handleMarkRead = () => {
    ...
    setIsRead(true);
    if (next) {
      router.push(`/agent/learning/${next.lesson.slug}`);
    }
  };
    ...
    <>
      <PageHeader
        ...
        breadcrumbs={[
          ...
          { title: "Обучение", href: "/agent/learning" },
          { title: mod.title },
        ]}
      />
      ...
      <div className="flex items-center gap-3 mb-6 text-sm text-muted-foreground">
        ...
        {isRead && (
          <span className="flex items-center gap-1 text-green-500">
            <CheckCircle2 className="h-3.5 w-3.5" /> Прочитано
          </span>
        )}
      </div>
      ...
      <div className="flex items-center justify-between">
        <div>
          {prev && (
            <Link href={`/agent/learning/${prev.lesson.slug}`}>
              <Button variant="outline" size="sm">
                <ChevronLeft className="h-4 w-4 mr-1" /> {prev.lesson.title}
              </Button>
            </Link>
          )}
        </div>
        <div className="flex items-center gap-2">
          {!isRead && (
            <Button size="sm" onClick={handleMarkRead}>
              ...
              {next ? "Прочитано — далее" : "Прочитано"}
            </Button>
          )}
          {isRead && next && (
            <Link href={`/agent/learning/${next.lesson.slug}`}>
              <Button size="sm">
