...
export default function ManagerProfilePage() {
  ...
  const handleTgConnect = async () => {
    setTgLoading(true);
    try { const r = await fetch("/api/telegram/link", { method: "POST" }); if (r.ok) { const d = await r.json(); setTgDeepLink(d.deepLink || d.linkUrl); } }
  ...
  return (
    <div>
      <PageHeader
        ...
        breadcrumbs={[{ title: "О платформе", href: "/manager/dashboard" }, { title: "Профиль" }]}
      />
            ...
            <CardContent className="p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                ...
                <div>
                  ...
                  <Input value={form.profession} onChange={(e) => setForm((f) => ({ ...f, profession: e.target.value }))} placeholder="Юрист, HR, менеджер..." />
                </div>
              </div>
