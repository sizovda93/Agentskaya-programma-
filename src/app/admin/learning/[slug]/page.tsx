...
const STORAGE_KEY = "learning_read_admin";
...
export default function AdminLessonPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const result = getLesson("admin", slug);
  const allLessons = getAllLessons("admin");
  ...
  if (!result) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        ...
        <Button variant="outline" onClick={() => router.push("/admin/learning")}>
  ...
  const handleMarkRead = () => {
    markRead(slug);
    setIsRead(true);
    if (next) router.push(`/admin/learning/${next.lesson.slug}`);
  };
    ...
    <>
      <PageHeader
        ...
        breadcrumbs={[
          ...
          { title: "Обучение", href: "/admin/learning" },
          { title: mod.title },
        ]}
      />
      ...
      <div className="flex items-center justify-between">
        <div>
          {prev && (
            <Link href={`/admin/learning/${prev.lesson.slug}`}>
        ...
        <div className="flex items-center gap-2">
          ...
          {isRead && next && (
            <Link href={`/admin/learning/${next.lesson.slug}`}>
